...
def return_date_list(frequency,filter_dict):
    ...
    if frequency=="Yesterday":
        ...
        for i in range(4):
            add=i+1
            date_value_list.append(date(date.today().year - add,1,1).strftime('%Y-%m-%d')) 
        ...
        for key in filter_dict:
                if key=="end_date":
                    if type(filter_dict[key])==list:
                        ...
                        date_value_list=quarter_cycle(quarter_list,date_value_list,datee.year-1,datee)
                        date_value_list=quarter_cycle(quarter_list,date_value_list,datee.year-2,datee)
...
#     date_value_list.reverse()
    if report_type=="Monthly HC":
        ...
        # final_dataframe["Net Addition"]=0
        for date_value in date_value_list:
                    given_date = datetime(year=2020, month=int(date_value.split('-')[1]), day=1).date()
                    if frequency=="Annually":
                            from datetime import date
                    ...
                    # final_dataframe["Net Addition"]=final_dataframe.sum(numeric_only=True, axis=0)
        # final_dataframe.loc['Net Addition']= final_dataframe.sum(numeric_only=True, axis=0)
        final_dataframe.loc[len(final_dataframe.index)] = final_dataframe.sum(numeric_only=True, axis=0)
