    ...
    if report_type == "Monthly HC":
        if frequency == "MTD":
            for date_value in date_value_list:
                ...
                filtered_df = EmployeeMaster[
                    (EmployeeMaster['DOJ'] <= end_of_month) & (EmployeeMaster['LWD'] >= date_value)]
                print(date_value)
        ...
        for date_value in date_value_list:
            ...
            end_of_month = pd.to_datetime(end_of_month).strftime('%Y-%m-%d')
            filtered_df = EmployeeMaster[
                (EmployeeMaster['DOJ'] <= end_of_month) & (EmployeeMaster['LWD'] >= end_of_month)]
            print(date_value)
            for index, row in final_dataframe.iterrows():
                count = 0
                filter_dict = {"VendorName": row["VendorName"]}
        ...
        for date_value in date_value_list:
            ...
            end_of_month = pd.to_datetime(end_of_month).strftime('%Y-%m-%d')
            filtered_df = EmployeeMaster[
                (EmployeeMaster['DOJ'] >= date_value) & (EmployeeMaster['DOJ'] <= end_of_month)]
            for index, row in final_dataframe.iterrows():
                count = 0
                filter_dict = {"VendorName": row["VendorName"]}
                for key in filter_dict:
        ...
        for date_value in date_value_list:
            ...
            end_of_month = pd.to_datetime(end_of_month).strftime('%Y-%m-%d')
            filtered_df = EmployeeMaster[
                (EmployeeMaster['LWD'] >= date_value) & (EmployeeMaster['LWD'] <= end_of_month)]
        ...
        for date_value in date_value_list:
            ...
            end_of_month = pd.to_datetime(end_of_month).strftime('%Y-%m-%d')
            exit_count = len(
                EmployeeMaster[(EmployeeMaster['LWD'] >= date_value) & (EmployeeMaster['LWD'] <= end_of_month)])
            opening_count = len(
                EmployeeMaster[(EmployeeMaster['DOJ'] <= date_value) & (EmployeeMaster['LWD'] >= date_value)])
            closing_count = len(
                EmployeeMaster[(EmployeeMaster['DOJ'] <= end_of_month) & (EmployeeMaster['LWD'] >= end_of_month)])
            ...
            for index, row in final_dataframe.iterrows():
                ...
                for key in filter_dict:
                    ...
                    exit_count = len(
                        filtered_df[(filtered_df['LWD'] >= date_value) & (filtered_df['LWD'] <= end_of_month)])
                    opening_count = len(
                        filtered_df[(filtered_df['DOJ'] <= date_value) & (filtered_df['LWD'] >= date_value)])
                    closing_count = len(
                        filtered_df[(filtered_df['DOJ'] <= end_of_month) & (filtered_df['LWD'] >= end_of_month)])
                    try:
